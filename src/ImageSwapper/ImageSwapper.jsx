...
const ImageSwapper = ({ num, setCount }) => {
    ...
    useEffect(
        () => {
            console.log("UseEffect of ImageSwapper is running");

            return () => {
                console.log("ImageSwapper is dying (unmount)");
            }
        },
        []
    )
    ...
    return (
        <>
            <div> Num is: {num} </div>
            {num === 1 && <h3> I'm not showing the image now! </h3>}
            {num !== 1 && <img onClick={() => { setCount(0) }} src={src} alt="image1" />}
